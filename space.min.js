...
Space.prototype.getByString=function(a){if(a){if(!a.match(/ /))return this.values[a];a=a.split(/ /g);var b=a.shift();if(b in this.values&&this.values[b]instanceof Space)return this.values[b].get(a.join(" "))}};Space.prototype.getBySpace=function(a){var b=new Space,c;for(c in a.keys){var d=a.keys[c],e=this.values[d];"undefined"!==typeof e&&(!(a.values[d]instanceof Space)||!a.values[d].keys.length?b._set(d,e):e instanceof Space&&b._set(d,e.getBySpace(a.values[d])))}return b};
...
Space.prototype.getTokensConcise=function(){return this.getTokens().replace(/[^\w\s]|(.)(?=\1)/gi,"")};Space.prototype._keyCount=function(){var a=this.length();this.each(function(b,c){c instanceof Space&&(a+=c._keyCount())});return a};Space.prototype.length=function(){return this.keys.length};Space.prototype.next=function(a){a=this.keys.indexOf(a)+1;return this.keys[a]?this.keys[a]:this.keys[0]};
...
Space.prototype.pop=function(){if(!this.keys.length)return null;var a=this.keys.pop(),b=new Space;b.set(a,this.get(a));this._delete(a);return b};Space.prototype.prev=function(a){a=this.keys.indexOf(a)-1;return 0<=a?this.keys[a]:this.keys[this.keys.length-1]};Space.prototype.push=function(a){for(var b=this.length();this.get(b.toString());)b++;this._set(b.toString(),a);return this};
Space.prototype._rename=function(a,b){this.values[b]=this.values[a];delete this.values[a];this.keys[this.keys.indexOf(a)]=b;return this};Space.prototype.rename=function(a,b){var c=Space.pathBranch(a),d=Space.pathLeaf(a),e=b,f=this;c&&(f=this.get(c),e=b.substr(c.length+1));f._rename(d,e);a!==b&&this.trigger("rename",a,b);this.trigger("change");return this};
