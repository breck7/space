function Space(a){this._pairs=[];this._index={};this._cache={};this.events={};this._load(a);return this}Space.version="0.9.1";Space.removeItems=function(a,b){var c=[];"number"===typeof b&&(b=[b]);for(var d=b.length-1;0<=d;d--)c.push(a.splice(b[d],1));return c};Space.isXPath=function(a){return a.match(/ /)};
...
Space.fromDelimiter=function(a,b){for(var c=a.length,d="",e=0,f=!1,h=[[]],g=new Space,j=0;e<c;){var l=a[e];f?'"'!==l?d+=l:'"'!==a[e+1]?f=!1:(d+='"',e++):l===b?(h[j].push(d),d="",'"'===a[e+1]&&(f=!0,e++)):"\n"===l?(h[j].push(d),d="",j++,a[e+1]&&(h[j]=[]),'"'===a[e+1]&&(f=!0,e++)):e===c-1?h[j].push(d+l):d+=l;e++}for(c=0;c<h[0].length;c++)h[0][c]=h[0][c].replace(/ /g,"");h.forEach(function(a,b){if(0===b)return!0;var c=new Space;h[0].forEach(function(b,d){var e=a[d];if(""===e)return!0;c.append(b,e)});
g.append(b-1,c)});return g};Space.fromSsv=function(a){return Space.fromDelimiter(a," ")};Space.fromTsv=function(a){return Space.fromDelimiter(a,"\t")};Space.toFile=function(a,b,c,d,e){if("undefined"===typeof require)throw Error("toFile only works with node.js");"undefined"===typeof fs&&(fs=require("fs"));"function"===typeof c&&(d=c,c=null);return!e&&!d?fs.writeFileSync(a,b.toString(),c):fs.writeFile(a,b.toString(),c,d)};
...
new Space);e&&g[j]._setPair(b,c);return this};Space.prototype._loadFromString=function(a){a=a.replace(/^\s*/,"");a=a.replace("\r","");a=a.replace(/\n\n+/,"\n");a=a.split(/\n(?! )/g);var b,c;for(c in a){var d=a[c];(b=d.match(/^([^ ]+)(\n|$)/))?this._setPair(b[1],new Space(d.substr(b[1].length).replace(/\n /g,"\n"))):(b=d.match(/^([^ ]+) /))&&this._setPair(b[1],d.substr(b[1].length+1).replace(/\n /g,"\n"))}return this};
Space.prototype.mapProperties=function(a,b){this._index={};this._cache={};for(var c=this._pairs.length,d=0;d<c;d++)this._pairs[d][0]=a(this._pairs[d][0]),b&&this._pairs[d][1]instanceof Space&&this._pairs[d][1].mapProperties(a,b),this._updateCache(this._pairs[d][0],this._pairs[d][1]);return this};
Space.prototype.mapValues=function(a,b){this._index={};this._cache={};for(var c=this._pairs.length,d=0;d<c;d++)this._pairs[d][1]instanceof Space?b&&this._pairs[d][1].mapValues(a,b):this._pairs[d][1]=a(this._pairs[d][1]),this._updateCache(this._pairs[d][0],this._pairs[d][1]);return this};Space.prototype.next=function(a){a=this.indexOf(a)+1;return this._getPropertyByIndex(a)};
...
Space.prototype._toObject=function(){var a=this.getProperties(),b=0<a.length,c=0;a.forEach(function(a){a!==c.toString()&&(b=!1);c++});var d=b?[]:{};this.each(function(a,c){var h;h=c instanceof Space?c._toObject():"false"===c?!1:"true"===c?!0:"null"===c?null:c.match(/^[\-\.]?[0-9]+[0-9\.]*$/)?parseFloat(c):c;b?d.push(h):d[a]=h});return d};Space.prototype.toObject=function(a){if(a)return this._toObject();var b={};this.each(function(a,d){b[a]=d instanceof Space?d.toObject():d});return b};
...
Space.prototype.toXML=function(a){return this._toXML(a?0:-1)};Space.prototype._toXML=function(a){var b="",c=-1===a?"":Space.strRepeat(" ",a);this.each(function(d,e){b+=c+"<"+d+">";e instanceof Space?0<e.length()&&(b+=(-1===a?"":"\n")+e._toXML(-1<a?a+2:-1)+c):b+=e;b+="</"+d+">"+(-1===a?"":"\n")});return b};Space.prototype.toXMLWithAttributes=function(a){return this._toXMLWithAttributes(a?0:-1)};
Space.prototype._toXMLWithAttributes=function(a){var b="",c=-1===a?"":Space.strRepeat(" ",a);this.each(function(d,e){var f=e.get("content"),h=e.get("attributes"),g="",j="";h&&h.each(function(a,b){g+=" "+a+'="'+b.replace('"','\\"')+'"'});f&&(f instanceof Space?0<f.length()&&(j=(-1===a?"":"\n")+f._toXMLWithAttributes(-1<a?a+2:-1)+c):j=f);b+=c+"<"+d+g+">"+j+"</"+d+">"+(-1===a?"":"\n")});return b};
