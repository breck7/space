function Space(a){this._pairs=[];this._index={};this._cache={};this.events={};this._load(a);return this}Space.version="0.8.9";Space.removeItems=function(a,b){var c=[];"number"===typeof b&&(b=[b]);for(var d=b.length-1;0<=d;d--)c.push(a.splice(b[d],1));return c};Space.isXPath=function(a){return a.match(/ /)};
Space.fromHeredoc=function(a,b,c){a=a.replace(/\n\r/g,"\n");a=a.split("\n");var d=null,e=[],f=RegExp("^"+b+"(?: |$)");c=RegExp("^"+c);for(var g=0;g<a.length;g++)null===d?a[g].match(f)&&(d=g,a[g].length===b.length&&(a[g]+=" ")):a[g].match(c)?(d=null,e.push(g)):a[g]=" "+a[g];Space.removeItems(a,e);return new Space(a.join("\n"))};
...
Space.prototype._deleteByIndex=function(a){if(void 0===this._pairs[a])return 0;var b=this._pairs.splice(a,1),c=b[0][0];this._index[c]--;if(this._index[c]){if(b[0][1]===this._cache[c])for(;a<this.length();a++)if(this._pairs[a][0]===c){this._cache[c]=this._pairs[a][1];break}}else delete this._cache[c],delete this._index[c];return 1};Space.prototype._deleteByProperty=function(a){a=this.indexOf(a);return-1===a?0:this._deleteByIndex(a)};
...
Space.prototype.getTokens=function(){for(var a=this.toString(),b="K",c="",d=1,e=0,f=0;f<a.length-1;f++){var g=a.substr(f,1);0<e?(c+="E",e--):" "!==g&&"\n"!==g?("N"===b&&(b="K"),c+=b):" "===g?"V"===b?c+=b:"K"===b?(c+="S",b="V"):(d++,c+="N"):"K"===b?(b="N",d=1,c+="N"):"V"===b&&(a.substr(f+1,d)===Space.strRepeat(" ",d)?(c+="V",e=d):(b="N",d=1,c+="N"))}return c};Space.prototype.getTokensConcise=function(){return this.getTokens().replace(/[^\w\s]|(.)(?=\1)/gi,"")};
Space.prototype.getValues=function(){var a=[];this._pairs.forEach(function(b){a.push(b[1])});return a};Space.prototype.has=function(a){return!!this._getValueByProperty(a)};Space.prototype.__height=function(){return this.toString().match(/\n/g).length};Space.prototype.indexOf=function(a){if(!this._index[a])return-1;for(var b=0;b<this.length();b++)if(this._pairs[b][0]===a)return b;return-1};Space.prototype.insert=function(a,b,c){this._setPair(a,b,c);return this};
...
Space.prototype._loadFromString2=function(a){for(var b="",c="",d="",e="",f=0,g=0,h={"0":this},l=0,k=0,n=!1,m=0;m<a.length;m++){var j=a[m];if("\r"!==j&&(n||!("\n"===j||" "===j)))n||(n=d=!0),d?" "===j?(d=!1,e=!0,l=f>=g?g:f):"\n"===j?(d=!1,f>=g&&(f=g),g=f+1,h[g]=new Space,h[f]._setPair(b,h[g]),f=0,b=""):b+=j:e?k&&" "===j?(f++,k--):k?(h[l]._setPair(b,c.substr(0,c.length-1)),c="",d=!0,e=!1,b=j,k=0):"\n"===j?(k=l+1,f=0,m!==a.length-1&&(c+=j)):c+=j:"\n"!==j&&(" "===j?f++:(d=!0,b=j))}d&&h[f]._setPair(b,new Space);
e&&h[l]._setPair(b,c);return this};Space.prototype._loadFromString=function(a){a=a.replace(/^\s*/,"");a=a.replace(/\n\r/g,"\n");a=a.replace(/\n\n+/,"\n");a=a.split(/\n(?! )/g);var b,c;for(c in a){var d=a[c];(b=d.match(/^([^ ]+)(\n|$)/))?this._setPair(b[1],new Space(d.substr(b[1].length).replace(/\n /g,"\n"))):(b=d.match(/^([^ ]+) /))&&this._setPair(b[1],d.substr(b[1].length+1).replace(/\n /g,"\n"))}return this};Space.prototype.next=function(a){a=this.indexOf(a)+1;return this._getPropertyByIndex(a)};
...
Space.prototype._setByXPath=function(a,b){if(!a)return null;for(var c=a.toString().split(/ /g),d=this,e,f,g=0;g<c.length;g++){e=c[g];if((f=g===c.length-1)||!(d._getValueByProperty(e)instanceof Space)){var h;h=f?b:new Space;d.has(e)?(f=d.indexOf(e),d._setPair(e,h,f,!0)):d._setPair(e,h)}d=d.get(e)}return this};
...
Space.prototype.shift=function(){if(!this.length())return null;var a=this._getPropertyByIndex(0),b=new Space;b.set(a,this.get(a));this._deleteByIndex(0);return b};Space.prototype.sort=function(a){this._pairs=this._pairs.sort(a);return this};Space.prototype.split=function(a){var b=[],c=!1,d=new Space;this.each(function(e,f){e===a&&(c?b.push(d):c=!0,d=new Space);if(!c)return!0;d.append(e,f)});c&&b.push(d);return b};Space.prototype.tableOfContents=function(){return this.getProperties().join(" ")};
...
Space.prototype.toColumns=function(){for(var a=this.toString(),b=[],c=0,d=0,e=0,f=0;f<a.length;f++){var g=a[f];if("\n"===g){for(;c<d;)b[c]+=" ",c++;c=0;e++}else{if(!b[c]){b[c]="";for(var h=0;h<e;h++)b[c]+=" "}b[c]+=g;c++;c>d&&(d=c)}}return b};Space.prototype.toDecimalMatrix=function(){for(var a=this.__width(),b=this.toString().replace(/\n$/,"").split(/\n/g),c=[],d=0;d<b.length;d++){for(var e=b[d],f=[],g=e.length,h=0;h<a;h++)h<g?f.push(e.substr(h,1).charCodeAt(0)):f.push(0);c.push(f)}return c};
Space.prototype.toDecimalMatrixString=function(){var a="";this.toDecimalMatrix().forEach(function(b){var c="";b.forEach(function(b){a+=c;a=10>b?a+("00"+b):9<b&&100>b?a+("0"+b):a+b;c=" "});a+="\n"});return a};Space.prototype.toggle=function(a,b,c){this.get(a)===b?this.set(a,c):this.set(a,b);return this};Space.prototype.toJavascript=function(a){var b="new Space('"+this.toString().replace(/\n/g,"\\n").replace(/\'/g,"\\'")+"')";return a?b.replace(/\\n/g,"\\n\\\n"):b};Space.prototype.toJSON=function(){return JSON.stringify(this.toObject())};
