...
Space.prototype._loadFromString2=function(a){for(var b="",c="",d="",e="",f=0,h=0,g={"0":this},k=0,l=0,n=!1,p=a.length,m=0;m<p;m++){var j=a[m];if("\r"!==j&&(n||!("\n"===j||" "===j)))n||(n=d=!0),d?" "===j?(d=!1,e=!0,k=f>=h?h:f):"\n"===j?(d=!1,f>=h&&(f=h),h=f+1,g[h]=new Space,g[f]._setPair(b,g[h]),f=0,b=""):b+=j:e?l&&" "===j?(f++,l--):l?(g[k]._setPair(b,c.substr(0,c.length-1)),c="",d=!0,e=!1,b=j,l=0):"\n"===j?(l=k+1,f=0,m!==a.length-1&&(c+=j)):c+=j:"\n"!==j&&(" "===j?f++:(d=!0,b=j))}d&&g[f]._setPair(b,
new Space);e&&g[k]._setPair(b,c);return this};Space.prototype._loadFromString=function(a){a=a.replace(/^\s*/,"");a=a.replace(/\n\r/g,"\n");a=a.replace(/\n\n+/,"\n");a=a.split(/\n(?! )/g);var b,c;for(c in a){var d=a[c];(b=d.match(/^([^ ]+)(\n|$)/))?this._setPair(b[1],new Space(d.substr(b[1].length).replace(/\n /g,"\n"))):(b=d.match(/^([^ ]+) /))&&this._setPair(b[1],d.substr(b[1].length+1).replace(/\n /g,"\n"))}return this};Space.prototype.next=function(a){a=this.indexOf(a)+1;return this._getPropertyByIndex(a)};
...
Space.prototype._setByXPath=function(a,b){if(!a)return null;for(var c=a.toString().split(/ /g),d=this,e,f,h=c.length,g=0;g<h;g++){e=c[g];if((f=g===c.length-1)||!(d._getValueByProperty(e)instanceof Space)){var k;k=f?b:new Space;d.has(e)?(f=d.indexOf(e),d._setPair(e,k,f,!0)):d._setPair(e,k)}d=d.get(e)}return this};
...
Space.prototype.toColumns=function(){for(var a=this.toString(),b=[],c=0,d=0,e=0,f=a.length,h=0;h<f;h++){var g=a[h];if("\n"===g){for(;c<d;)b[c]+=" ",c++;c=0;e++}else{if(!b[c]){b[c]="";for(var k=0;k<e;k++)b[c]+=" "}b[c]+=g;c++;c>d&&(d=c)}}return b};Space.prototype.toDecimalMatrix=function(){for(var a=this.__width(),b=this.toString().replace(/\n$/,"").split(/\n/g),c=[],d=0;d<b.length;d++){for(var e=b[d],f=[],h=e.length,g=0;g<a;g++)g<h?f.push(e.substr(g,1).charCodeAt(0)):f.push(0);c.push(f)}return c};
