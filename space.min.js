function Space(a){this._load(a)}Space.version="0.21.3";Space._isSpacePath=function(a){return-1<a.indexOf(" ")};Space.fromHeredoc=function(a,b,c){a=a.replace(/\n\r/g,"\n");a=a.split("\n");var d=[],e=new RegExp("^"+b+"(?: |$)"),f=a.length;b=b.length;c=new RegExp("^"+c);for(var g=null,h=0;h<f;h++)null===g?a[h].match(e)&&(g=h,a[h].length===b&&(a[h]+=" ")):a[h].match(c)?(g=null,d.push(h)):a[h]=" "+a[h];Space._removeItems(a,d);return new Space(a.join("\n"))};
Space.fromCsv=function(a,b){return Space.fromDelimiter(a,",",b)};
Space.fromDelimiter=function(a,b,c,d){!1!==d&&-1<a.indexOf("\r")&&(a=a.replace(/\r/g,""));d=[[]];if(-1<a.indexOf('"'))for(var e=a.length,f="",g='"'===a.substr(0,1),h=g?1:0,k,n,m=0,l,p;h<e;)l=a[h],n=h+1===e,k=a[h+1],p='"'===k,g?'"'!==l?f+=l:p?(f+=k,h++):(g=!1,n&&d[m].push(f)):l===b?(d[m].push(f),f="",p&&(g=!0,h++)):"\n"===l?(d[m].push(f),f="",m++,k&&(d[m]=[]),p&&(g=!0,h++)):n?d[m].push(f+l):f+=l,h++;else for(a=a.split(/\n/g),e=a.length,f=0;f<e;f++)a[f]&&(d[f]=a[f].split(b));a=d[0].length;h=(c=!1===
c?!1:!0)?d[0]:[];if(c)for(e=0;e<a;e++)h[e]=h[e].replace(/ /g,"");else for(e=0;e<a;e++)h.push(e);e=new Space;f=[];g=[];h={properties:h};k=d.length;n=0;for(c=c?1:0;c<k;c++){m=new Space;l=d[c];if(l.length>a)l[a-1]=l.slice(a-1).join(b),l=l.slice(0,a);else if(l.length<a)for(;l.length<a;)l.push("");m.setWithType(h,l);f.push(n);g.push(m);n++}e.setWithType({properties:f,index:f},g);return e};
...
Space.prototype._every=function(a,b){var c=this,d=!0;this.each(function(e,f,g){var h=f instanceof Space;h&&b||(d=a.call(c,e,f,g));if(!1===d)return!1;h&&(d=f._every(a,b));return d});return d};Space.prototype.extract=function(a){a=a.split(" ");var b={},c=new Space;a.forEach(function(a){b[a]=!0});this._extract(b,c);return c};Space.prototype._extract=function(a,b){this.each(function(c,d){a[c]?b.append(c,d):d instanceof Space&&d._extract(a,b)})};
...
Space.prototype.map=function(a,b,c,d){if(!d)return(new Space(this)).map(a,b,c);for(var e=this.length,f=this._getValues(),g=this._getProperties(),h=0;h<e;h++){var k=g[h];a&&(g=this._setProperty(h,a(k)));c&&f[h]instanceof Space?f[h].map(a,b,c,d):b&&(f[h]=b(f[h],g[h],k))}delete this._index;return this};
Space.prototype.mergeDuplicates=function(){var a={},b=[],c=this;this.each(function(d,e,f){if(!(e instanceof Space))return!0;void 0===a[d]?a[d]=f:(e.each(function(b,f){c.at(a[d]).set(b,f)}),b.push(f))});return this._deleteByIndexes(b)};Space.prototype.nest=function(a){var b=new Space;b.set(a,this);return this.reload(b.toString())};Space.prototype.next=function(a){return this._getPropertyByIndex(this.indexOf(a)+1)};
...
Space.prototype._setBySpacePath=function(a,b){a=a?this._sanitizeSpacePath(a):!1;if(!a)return null;for(var c=a.split(/ /g),d=c.length,e=this,f,g,h,k=0;k<d;k++)f=c[k],!(g=k===c.length-1)&&e._getValueByProperty(f)instanceof Space||(h=g?b:new Space,e.has(f)?(g=e.indexOf(f),e._setPair(f,h,g,!0)):e._setPair(f,h)),e=e.get(f);return this};
...
Space.prototype.toDelimited=function(a,b){var c=new RegExp('(\\n|\\"|\\'+a+")"),d=function(a){return a.match(c)?'"'+a.replace(/\"/g,'""')+'"':a},e="";b=b||this.getUnionType().properties;b.forEach(function(b){e+=a+d(b)});e=e.substr(1)+"\n";this.each(function(c,g){if(!(g instanceof Space))return!0;var h="";b.forEach(function(b){b=g.get(b)||"";h+=a+d(b.toString())});e+=h.substr(1)+"\n"});return e};
Space.prototype.toArrayWithHeader=function(a){if(!this.length)return[];var b=this._getValues();if(!a&&(this.each(function(b,c){if(a=c instanceof Space&&c.getType())return!1}),!a))return[];for(var c=this.length,d=[a.properties],e=0;e<c;e++)b[e]._type===a&&d.push(b[e]._getValues());return d};Space.prototype.toggle=function(a,b,c){this.get(a)===b?void 0===c?this.delete(a):this.set(a,c):this.set(a,b);return this};
